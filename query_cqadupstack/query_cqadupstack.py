...
class Subforum():
    ...
    def get_posts_dups_nodups_and_related(self):
        """ 
        Takes no input and return three lists: one with all posts that have at least one duplicate,
        one with all posts that have at least one related question, and one with all posts that don't
        have any duplicates or related questions. In that order.
    ...
    def get_random_postid(self):
        ...
        """
        allids = list(self.postdict.keys())
    ...
    def get_all_postids(self):
        ...
        """
        return list(self.postdict.keys())
    ...
    def get_post_title_and_body(self, postid):
        """
        Takes a post id as input and returns the title and the body of the post together as one string,
        so in other words, the full initial post.
        """
    ...
    def get_first_duptagdate(self, postid):
        """
        Takes one postid as input and returns the date on which this post was tagged as a duplicate for the first time,
        in %Y-%m-%dT%H:%M:%S.%f format,
    ...
    def get_answer_parentid(self, answerid):
        """
        Takes an answer id as input and returns its parent id:
        the id of the post it is an answer of
        """
    ...
    def get_acceptedanswer_date(self, answerid):
        """
        Takes an answer id as input and returns the date at which it was selected as the best answer in YYYY-MM-DD format,
        if it exists. Else it returns 0.
        """
    ...
    def get_answerbody(self, answerid):
        """
        Takes an answer id as input and returns the body of the answer.
        That is the text of the answer.
        """
    ...
    def get_commentscore(self, commentid):
        """
        Takes a comment id as input and returns an integer representing the score of the comment.
        This is the number of upvotes minus the number of downvotes is has received.
        """
    ...
    def _read_scorefile(self, scorefile):
        ...
        for line in inputf:
            ...
            # r ankedresults = rankedresults_evens
        ...
        for i, ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:
                ...
                if include_related_posts:
                    relevance_judgements = [1.0 if x in relevantdocs else (
                        0.5 if x in relateddocs else 0.0) for x in ranking]
                else:
                    relevance_judgements = [
                        1.0 if x in relevantdocs else 0.0 for x in ranking]
