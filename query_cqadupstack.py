...
class Subforum():
    ...
    def _general_cleaning(self, s, codes, remove_punct=False):
        ...
        # restore URLs
        newurllist = itertools.izip(reversed(range(len(newurls))), reversed(newurls)) # reverse list to GGGG1 does not match GGG10. (Source: http://galvanist.com/post/53478841501/python-reverse-enumerate)
        ...
        # restore codeblocks
        newlist = itertools.izip(reversed(range(len(codes))), reversed(codes)) # reverse list to hhhh1 does not match hhhh10 (Source: http://galvanist.com/post/53478841501/python-reverse-enumerate)
        for i, c in newlist:
        ...
        for i,ranking in enumerate(all_rankings):
            ...
            if relevantdocs != []:# or (relateddocs != [] and include_related_posts): # We only compute the score for queries that actually do have duplicates in the indexed set.
                ...
                if include_related_posts:
                    relevance_judgements = [1.0 if x in relevantdocs else (0.5 if x in relateddocs else 0.0) for x in ranking]
                else:
                    relevance_judgements = [1.0 if x in relevantdocs else 0.0 for x in ranking]
